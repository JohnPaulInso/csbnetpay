...
graph TD
    ...
    D --> E[Fetch initial batch of 6 months]
    E --> F{For each month...}
...
### 1. Sequential Pagination (6-Month Initial + 3-Month Load More)
...
- **Initial load fetches 6 months upfront** (changed from 3 on 2026-07-10) so past 6 months are visible without clicking Load More.
- Renders an inline **"Load More"** button matching the theme color with gradient backgrounds, box-shadows, and smooth translate hover animations that loads 3 more months at a time.
...
- **Uses `data-expanded` attribute** (changed from `style.display === 'none'` check on 2026-07-10) for reliable toggle across ALL month rows.
- A chevron arrow icon (`bi-chevron-right` = collapsed, `bi-chevron-down` = expanded) visually signals the collapsed/expanded state.
...
- All month rows — not just the first — can be independently toggled open/closed.
...
| Date | Fix | File |
|------|-----|------|
| 2026-07-10 | Initial batch: 3 → 6 months | `index3.script` |
| 2026-07-10 | Accordion toggle: style.display check → data-expanded attribute (fixes all rows being toggleable) | `index3.script` |
| 2026-07-10 | Arrow icon: `bi-chevron-left` (wrong) → `bi-chevron-right` (collapsed) / `bi-chevron-down` (expanded) | `index3.script` |
